"""Native adapter for Strands Agents (tool_context.interrupt human-in-the-loop).

A Strands tool pauses via tool_context.interrupt(); the human response lives in the
agent's interrupt state and is trusted on resume. A writer of that persisted state can
...
The tool context exposes no trusted run identity, so Vyrion binds the Seal to action +
arguments + a single-use nonce, carried by the trusted approver as the interrupt
response, and verifies it inside the tool before the side effect.
"""
...
from ..protection.patcher import patch_strands
...
class StrandsNativeAdapter(NativeAdapter):
    framework = "Strands Agents"
    native_package = "vyrion-strands"
    host_requirements = "pip install strands-agents"
        ...
        for path in self._py_files(project):
            ...
            for n in ast.walk(tree):
                if isinstance(n, (ast.Import, ast.ImportFrom)):
                    ...
                    if mod == "strands" or mod.startswith("strands."):
                        hits.append(self._ev(path, n, text, "strands import"))
        return DetectionEvidence(detected=bool(hits), framework=self.framework, evidence=hits)

    def detect_version(self, project):
        try:
            import importlib.metadata as m
            return m.version("strands-agents")
        except Exception:
            return "unknown"

    def _tool_shape(self, project):
        """Find the tool function that calls tool_context.interrupt(), the response
        variable, and its domain args. Returns
        (path, action_func, interrupt_var, arg_fields, source_rel)."""
        for path in self._py_files(project):
            text = Path(path).read_text()
            if ".interrupt(" not in text or "strands" not in text:
                continue
            try:
                tree = ast.parse(text)
            except SyntaxError:
                    ...
                    for stmt in ast.walk(n):
                        if isinstance(stmt, ast.Assign) and isinstance(stmt.value, ast.Call):
                            ...
                            if isinstance(f, ast.Attribute) and f.attr == "interrupt" \
                                    and stmt.targets and isinstance(stmt.targets[0], ast.Name):
                                interrupt_var = stmt.targets[0].id
                    if interrupt_var:
                        ctx_param = next((a.arg for a in n.args.args
                                          if a.arg == "tool_context"), None)
                        ...
                        domain = [a.arg for a in n.args.args if a.arg != ctx_param]
                        return (path, n.name, interrupt_var, (domain or ["amount"]),
    ...
    def discover_approval_points(self, project):
        ...
        for n in ast.walk(ast.parse(text)):
            if isinstance(n, (ast.FunctionDef, ast.AsyncFunctionDef)) and n.name == action:
                return [ApprovalPointEvidence("strands-approval", "tool_context.interrupt()",
                                              self._ev(path, n, text, f"interrupt tool {action}()"))]
        return []

    def discover_persistence(self, project):
        return [PersistenceEvidence("Strands agent interrupt state",
                                    "agent._interrupt_state.interrupts[...].response", True)]

    def discover_resume(self, project):
        return [ResumeEvidence("agent invoked with interrupt responses; interrupt() returns them")]

    def trace_actions(self, project, approval):
        s = self._tool_shape(project)
        if not s:
            return []
        path, action, interrupt_var, args, rel = s
        text = Path(path).read_text()
        for n in ast.walk(ast.parse(text)):
            if isinstance(n, (ast.FunctionDef, ast.AsyncFunctionDef)) and n.name == action:
                tr = ActionTrace(approval.id, f"action.{action}", "Strands interrupt tool",
                                 args, ["approval"] + args, self._ev(path, n, text, f"tool {action}()"))
                tr.action_func = action
                tr.interrupt_var = interrupt_var
                tr.source_rel = rel
                return [tr]
        return []
    ...
    def generate_patch(self, project, trace):
        ...
        manifest = {"schema": "vyrion.manifest.v1", "framework": "strands",
        ...
        plan = PatchPlan(
            steps=["back up the tool source",
                   "patch source: verify a provenance-bound Seal from the interrupt response",
        ...
        plan.interrupt_var = getattr(trace, "interrupt_var", "response")
    ...
    def apply_patch(self, project, patch):
        ...
        if target.exists():
            ...
            new_src = patch_strands(original, getattr(patch, "action_func", "transfer_funds"),
                                    getattr(patch, "interrupt_var", "response"),
    ...
    def verify_runtime(self, project, applied):
        import importlib.util
        if _framework_missing("strands"):
            return RuntimeProof(status="pending_live_host",
                host_requirements=self.host_requirements,
                live_command="vyrion certify --framework strands --live --project .",
                transcript="Strands is not installed on this host; live gates pending.")
        if not (Path(project) / "payment_agent.py").exists():
            return RuntimeProof(status="pending_live_host",
                host_requirements="run your own agent or tests against the patched tool",
                transcript="Guard wired into your interrupt tool: it verifies the interrupt "
                           "response as a provenance-bound Seal (action + args + single-use nonce; "
                           "no trusted run anchor). Run your agent to confirm a forged or replayed "
                           "response is blocked.")
        try:
    ...
    def _live(self, project):
        ...
        importlib.reload(pa)
        deco = pa.transfer_funds
        raw = getattr(deco, "_tool_func", None) or getattr(deco, "__wrapped__", None) or deco
        ...
        def mkseal(args=base):
            return broker.issue(approver=Approver(subject="cfo@corp"), decision="approve",
                ...
                workflow=WorkflowRef(system="strands", run_id="", checkpoint_id=""),
                execution=ExecutionRef(environment=ENV, audience=AUD),
                approval_point_id=APID).to_dict()

        class Ctx:
            def __init__(self, resp): self._resp = resp
            def interrupt(self, name, reason=None, response=None): return self._resp

        def invoke(seal, args=base):
            resp = _json.dumps(seal) if seal else "approve"
            try:
                return "settled" in str(raw(**args, tool_context=Ctx(resp)))
    ...
    def rollback(self, project, applied):
        ...
        pristine = Path(__file__).parent.parent.parent / "_fixtures" / "strands" / "payment_agent.py"
