"""Native adapter for AutoGen / Microsoft Agent Framework (save_state/load_state).

AutoGen serializes agent and team state via save_state / load_state as plain JSON
with no integrity check. A writer of that state can inject an approval (the MSRC
finding: append to message_buffer, set next_speaker_index) so a guarded tool runs
on resume. There is no trusted run identity in the serialized state, so Vyrion binds
the Seal to action + arguments + a single-use nonce, minted by the trusted approver
and carried with the approval, and verifies it inside the tool before the side effect.
"""
...
from ..protection.patcher import patch_autogen
...
class AutoGenNativeAdapter(NativeAdapter):
    framework = "AutoGen"
    native_package = "vyrion-autogen"
    host_requirements = "pip install autogen-agentchat autogen-core"
        ...
        for path in self._py_files(project):
            ...
            for n in ast.walk(tree):
                if isinstance(n, (ast.Import, ast.ImportFrom)):
                    ...
                    if mod.startswith("autogen_agentchat") or mod.startswith("autogen_core"):
                        hits.append(self._ev(path, n, text, "autogen import"))
        return DetectionEvidence(detected=bool(hits), framework=self.framework, evidence=hits)

    def detect_version(self, project):
        try:
            import importlib.metadata as m
            return m.version("autogen-agentchat")
        except Exception:
            return "unknown"
    ...
    def _tool_shape(self, project):
        ...
        for path in self._py_files(project):
            text = Path(path).read_text()
            if "autogen" not in text or "approval" not in text:
                continue
            try:
                tree = ast.parse(text)
            except SyntaxError:
    ...
    def discover_approval_points(self, project):
        ...
        for n in ast.walk(ast.parse(text)):
            if isinstance(n, (ast.FunctionDef, ast.AsyncFunctionDef)) and n.name == action:
                return [ApprovalPointEvidence("autogen-approval", "approval carried in serialized state",
                                              self._ev(path, n, text, f"tool {action}()"))]
        return []

    def discover_persistence(self, project):
        return [PersistenceEvidence("AutoGen save_state / load_state (JSON)",
                                    "agent_states[...] message_buffer", True)]

    def discover_resume(self, project):
        return [ResumeEvidence("load_state(state) replays the serialized approval")]
    ...
    def trace_actions(self, project, approval):
        ...
        for n in ast.walk(ast.parse(text)):
            if isinstance(n, (ast.FunctionDef, ast.AsyncFunctionDef)) and n.name == action:
                tr = ActionTrace(approval.id, f"action.{action}", "AutoGen tool",
    ...
    def generate_patch(self, project, trace):
        ...
        manifest = {"schema": "vyrion.manifest.v1", "framework": "autogen",
        ...
        plan = PatchPlan(
            steps=["back up the tool source",
                   "patch source: verify a provenance-bound Seal from the approval before the tool runs",
    ...
    def apply_patch(self, project, patch):
        ...
        if target.exists():
            ...
            new_src = patch_autogen(original, getattr(patch, "action_func", "transfer_funds"),
    ...
    def verify_runtime(self, project, applied):
        import importlib.util
        if _framework_missing("autogen_core"):
            return RuntimeProof(status="pending_live_host",
                host_requirements=self.host_requirements,
                live_command="vyrion certify --framework autogen --live --project .",
                transcript="AutoGen is not installed on this host; live gates pending.")
        if not (Path(project) / "payment_agent.py").exists():
            return RuntimeProof(status="pending_live_host",
                host_requirements="run your own agent or tests against the patched tool",
                transcript="Guard wired into your tool: it verifies the approval as a "
                           "provenance-bound Seal (action + args + single-use nonce; AutoGen "
                           "serializes state whole, so no trusted run anchor). Run your agent "
    ...
    def _live(self, project):
        ...
        importlib.reload(pa)
        tool = pa.transfer_tool  # the FunctionTool (guard wired into the function)
        ...
        def mkseal(args=base):
            return broker.issue(approver=Approver(subject="cfo@corp"), decision="approve",
                ...
                workflow=WorkflowRef(system="autogen", run_id="", checkpoint_id=""),
        ...
        async def invoke(seal, args=base):
            ...
            try:
                return "settled" in str(await tool.run_json(a, CancellationToken()))
        ...
        ok = asyncio.run(invoke(mkseal(base), args={"recipient": "acct-A", "amount": 999999, "currency": "USD"}))
    ...
    def rollback(self, project, applied):
        ...
        pristine = Path(__file__).parent.parent.parent / "_fixtures" / "autogen" / "payment_agent.py"
