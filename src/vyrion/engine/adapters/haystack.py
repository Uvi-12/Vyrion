"""Native adapter for Haystack Agents (human-in-the-loop tool confirmation).

Haystack's Agent HITL produces a ToolExecutionDecision (execute=True, final_tool_params)
that is serialized into the AgentSnapshot (deepset treats the snapshot as editable by
design). A writer of that snapshot can inject execute=True with no binding to approver,
action, arguments, or a single-use nonce, and the tool runs on resume.

Snapshots are serialized whole, so there is no trusted run identity to anchor to.
Vyrion binds the Seal to action + arguments + a single-use nonce, carried by the trusted
approver with the confirmation, and verifies it inside the tool before the side effect.
"""
...
from ..protection.patcher import patch_haystack
...
class HaystackNativeAdapter(NativeAdapter):
    framework = "Haystack"
    native_package = "vyrion-haystack"
    host_requirements = "pip install haystack-ai"
        ...
        for path in self._py_files(project):
            ...
            for n in ast.walk(tree):
                if isinstance(n, (ast.Import, ast.ImportFrom)):
                    ...
                    if mod == "haystack" or mod.startswith("haystack."):
                        hits.append(self._ev(path, n, text, "haystack import"))
        return DetectionEvidence(detected=bool(hits), framework=self.framework, evidence=hits)

    def detect_version(self, project):
        try:
            import importlib.metadata as m
            return m.version("haystack-ai")
        except Exception:
            return "unknown"

    def _tool_shape(self, project):
        """Find the guarded tool function (has an 'approval' parameter), its approval
        param, and its domain args. Returns (path, action_func, approval_param,
        arg_fields, source_rel)."""
        for path in self._py_files(project):
            text = Path(path).read_text()
            if "haystack" not in text or "approval" not in text:
                continue
            try:
                tree = ast.parse(text)
            except SyntaxError:
    ...
    def discover_approval_points(self, project):
        ...
        for n in ast.walk(ast.parse(text)):
            if isinstance(n, (ast.FunctionDef, ast.AsyncFunctionDef)) and n.name == action:
                return [ApprovalPointEvidence("haystack-approval", "tool confirmation (ToolExecutionDecision)",
                                              self._ev(path, n, text, f"tool {action}()"))]
        return []

    def discover_persistence(self, project):
        return [PersistenceEvidence("Haystack AgentSnapshot / PipelineSnapshot",
                                    "ToolExecutionDecision.execute", True)]

    def discover_resume(self, project):
        return [ResumeEvidence("AgentSnapshot replays the serialized ToolExecutionDecision")]
    ...
    def trace_actions(self, project, approval):
        ...
        for n in ast.walk(ast.parse(text)):
            if isinstance(n, (ast.FunctionDef, ast.AsyncFunctionDef)) and n.name == action:
                tr = ActionTrace(approval.id, f"action.{action}", "Haystack tool",
    ...
    def generate_patch(self, project, trace):
        ...
        manifest = {"schema": "vyrion.manifest.v1", "framework": "haystack",
        ...
        plan = PatchPlan(
            steps=["back up the tool source",
                   "patch source: verify a provenance-bound Seal from the confirmation before the tool runs",
    ...
    def apply_patch(self, project, patch):
        ...
        if target.exists():
            ...
            new_src = patch_haystack(original, getattr(patch, "action_func", "transfer_funds"),
    ...
    def verify_runtime(self, project, applied):
        import importlib.util
        if _framework_missing("haystack"):
            return RuntimeProof(status="pending_live_host",
                host_requirements=self.host_requirements,
                live_command="vyrion certify --framework haystack --live --project .",
                transcript="Haystack is not installed on this host; live gates pending.")
        if not (Path(project) / "payment_agent.py").exists():
            return RuntimeProof(status="pending_live_host",
                host_requirements="run your own Haystack agent or tests against the patched tool",
                transcript="Guard wired into your tool: it verifies the confirmation as a "
                           "provenance-bound Seal (action + args + single-use nonce; snapshots "
                           "are serialized whole, so no trusted run anchor). Run your agent to "
                           "confirm a forged or replayed confirmation is blocked.")
        try:
    ...
    def _live(self, project):
        ...
        importlib.reload(pa)
        tool = pa.transfer_tool  # the Haystack Tool (guard wired into the function)
        ...
        def mkseal(args=base):
            return broker.issue(approver=Approver(subject="cfo@corp"), decision="approve",
                ...
                workflow=WorkflowRef(system="haystack", run_id="", checkpoint_id=""),
                execution=ExecutionRef(environment=ENV, audience=AUD),
                approval_point_id=APID).to_dict()

        def blocked_exc(e):
            return "approval-provenance" in str(e) or "GhostApprovalBlocked" in type(e).__name__

        def invoke(seal, args=base):
            a = dict(args); a["approval"] = (_json.dumps(seal) if seal else "approve")
            try:
                return "settled" in str(tool.invoke(**a))
            except guardmod.GhostApprovalBlocked:
                return False
            except Exception as e:
                return not blocked_exc(e)  # Haystack wraps the guard error; treat as blocked
    ...
    def rollback(self, project, applied):
        ...
        pristine = Path(__file__).parent.parent.parent / "_fixtures" / "haystack" / "payment_agent.py"
