"""Native adapter for Agno (requires_confirmation tools).

An Agno tool declared requires_confirmation=True pauses for confirmation; the decision
(confirmed / confirmation_note) is serialized into the run/session state. A writer of
that state can set confirmed=True with no binding to approver, action, arguments, or a
single-use nonce, and the tool runs on resume (the Ghost Approval surface).

State is serialized whole, so there is no trusted run identity. Vyrion binds the Seal
to action + arguments + a single-use nonce, carried by the trusted approver with the
confirmation, and verifies it inside the tool before the side effect.
"""
...
from ..protection.patcher import patch_agno
...
class AgnoNativeAdapter(NativeAdapter):
    framework = "Agno"
    native_package = "vyrion-agno"
    host_requirements = "pip install agno"
    ...
    def _tool_shape(self, project):
        for path in self._py_files(project):
            text = Path(path).read_text()
            if "agno" not in text or "approval" not in text:
                continue
            try:
                tree = ast.parse(text)
            except SyntaxError:
    ...
    def discover_approval_points(self, project):
        ...
        for n in ast.walk(ast.parse(text)):
            if isinstance(n, (ast.FunctionDef, ast.AsyncFunctionDef)) and n.name == action:
                return [ApprovalPointEvidence("agno-approval", "requires_confirmation tool",
                                              self._ev(path, n, text, f"tool {action}()"))]
        return []

    def discover_persistence(self, project):
        return [PersistenceEvidence("Agno run/session state (ToolExecution)",
                                    "ToolExecution.confirmed", True)]

    def discover_resume(self, project):
        return [ResumeEvidence("run continued with the serialized ToolExecution confirmation")]
    ...
    def trace_actions(self, project, approval):
        ...
        for n in ast.walk(ast.parse(text)):
            if isinstance(n, (ast.FunctionDef, ast.AsyncFunctionDef)) and n.name == action:
                tr = ActionTrace(approval.id, f"action.{action}", "Agno tool",
    ...
    def generate_patch(self, project, trace):
        ...
        manifest = {"schema": "vyrion.manifest.v1", "framework": "agno",
    ...
    def apply_patch(self, project, patch):
        ...
        if target.exists():
            ...
            new_src = patch_agno(original, getattr(patch, "action_func", "transfer_funds"),
    ...
    def verify_runtime(self, project, applied):
        import importlib.util
        if _framework_missing("agno"):
            return RuntimeProof(status="pending_live_host",
                host_requirements=self.host_requirements,
                live_command="vyrion certify --framework agno --live --project .",
                transcript="Agno is not installed on this host; live gates pending.")
        if not (Path(project) / "payment_agent.py").exists():
            return RuntimeProof(status="pending_live_host",
                host_requirements="run your own agent or tests against the patched tool",
                transcript="Guard wired into your requires_confirmation tool: it verifies the "
                           "confirmation as a provenance-bound Seal (action + args + single-use "
                           "nonce; state serialized whole, so no trusted run anchor). Run your "
                           "agent to confirm a forged or replayed confirmation is blocked.")
        try:
    ...
    def _live(self, project):
        ...
        raw = getattr(deco, "entrypoint", None) or deco
        ...
        def mkseal(args=base):
            return broker.issue(approver=Approver(subject="cfo@corp"), decision="approve",
                ...
                workflow=WorkflowRef(system="agno", run_id="", checkpoint_id=""),
    ...
    def rollback(self, project, applied):
        ...
        pristine = Path(__file__).parent.parent.parent / "_fixtures" / "agno" / "payment_agent.py"
