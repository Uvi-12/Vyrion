"""Native adapter for Semantic Kernel Process framework (human-in-the-loop steps).

In the SK Process framework, human input arrives as an external event routed to a
step (external_event_target_map), and KernelProcessState is serialized to enable
replay. A writer of that checkpointed state can inject the approval event with no
binding to approver, action, arguments, or a single-use nonce, and the step runs on
replay (the Ghost Approval surface). SK's filter-based HITL is in-process and is a
resistant control, so Vyrion targets the Process-framework path.

Process state is serialized whole, so there is no trusted run identity. Vyrion binds
the Seal to action + arguments + a single-use nonce, carried by the trusted approver
in the external event, and verifies it inside the step before the side effect.
"""
...
from ..protection.patcher import patch_semantic_kernel
...
class SemanticKernelNativeAdapter(NativeAdapter):
    framework = "Semantic Kernel"
    native_package = "vyrion-semantic-kernel"
    host_requirements = "pip install semantic-kernel"
        ...
        for path in self._py_files(project):
            ...
            for n in ast.walk(tree):
                if isinstance(n, (ast.Import, ast.ImportFrom)):
                    ...
                    if mod == "semantic_kernel" or mod.startswith("semantic_kernel."):
                        hits.append(self._ev(path, n, text, "semantic-kernel import"))
        return DetectionEvidence(detected=bool(hits), framework=self.framework, evidence=hits)

    def detect_version(self, project):
        try:
            import importlib.metadata as m
            return m.version("semantic-kernel")
        except Exception:
            return "unknown"

    def _tool_shape(self, project):
        """Find the guarded process-step method (has an 'approval' parameter), its
        enclosing class, approval param, and domain args (excluding self). Returns
        (path, class_name, action_func, approval_param, arg_fields, source_rel)."""
        for path in self._py_files(project):
            text = Path(path).read_text()
            if "semantic_kernel" not in text or "approval" not in text:
                continue
            try:
                tree = ast.parse(text)
            except SyntaxError:
                ...
                for n in cls.body:
                    if isinstance(n, (ast.FunctionDef, ast.AsyncFunctionDef)):
                        ...
                        if approval_param:
                            ...
                            return (path, cls.name, n.name, approval_param, (domain or ["amount"]),
    ...
    def discover_approval_points(self, project):
        ...
        if not s:
            return []
        path, _, action, _, _, _ = s
        text = Path(path).read_text()
        for n in ast.walk(ast.parse(text)):
            if isinstance(n, (ast.FunctionDef, ast.AsyncFunctionDef)) and n.name == action:
                return [ApprovalPointEvidence("sk-approval", "Process external-event approval",
                                              self._ev(path, n, text, f"process step {action}()"))]
        return []

    def discover_persistence(self, project):
        return [PersistenceEvidence("SK KernelProcessState (serialized for replay)",
                                    "external event / step state", True)]

    def discover_resume(self, project):
        return [ResumeEvidence("process replayed from serialized KernelProcessState + external events")]

    def trace_actions(self, project, approval):
        s = self._tool_shape(project)
        if not s:
            return []
        path, class_name, action, approval_param, args, rel = s
        text = Path(path).read_text()
        for n in ast.walk(ast.parse(text)):
            if isinstance(n, (ast.FunctionDef, ast.AsyncFunctionDef)) and n.name == action:
                tr = ActionTrace(approval.id, f"action.{action}", "SK process step",
                                 args, ["approval"] + args, self._ev(path, n, text, f"step {action}()"))
    ...
    def generate_patch(self, project, trace):
        ...
        manifest = {"schema": "vyrion.manifest.v1", "framework": "semantic-kernel",
        ...
        plan = PatchPlan(
            steps=["back up the step source",
                   "patch source: verify a provenance-bound Seal from the external-event approval",
        ...
        plan.class_name = getattr(trace, "class_name", "PaymentStep")
    ...
    def apply_patch(self, project, patch):
        ...
        if target.exists():
            ...
            new_src = patch_semantic_kernel(original, getattr(patch, "action_func", "transfer_funds"),
    ...
    def verify_runtime(self, project, applied):
        import importlib.util
        if _framework_missing("semantic_kernel"):
            return RuntimeProof(status="pending_live_host",
                host_requirements=self.host_requirements,
                live_command="vyrion certify --framework semantic_kernel --live --project .",
                transcript="Semantic Kernel is not installed on this host; live gates pending.")
        if not (Path(project) / "payment_agent.py").exists():
            return RuntimeProof(status="pending_live_host",
                host_requirements="run your own SK process or tests against the patched step",
                transcript="Guard wired into your process step: it verifies the external-event "
                           "approval as a provenance-bound Seal (action + args + single-use nonce; "
                           "process state serialized whole, so no trusted run anchor). Run your "
                           "process to confirm a forged or replayed approval is blocked.")
        try:
    ...
    def _live(self, project):
        ...
        importlib.reload(pa)
        shape = self._tool_shape(project)
        class_name = shape[1] if shape else "PaymentStep"
        method_name = shape[2] if shape else "transfer_funds"
        step = getattr(pa, class_name)()
        ...
        def mkseal(args=base):
            return broker.issue(approver=Approver(subject="cfo@corp"), decision="approve",
                ...
                workflow=WorkflowRef(system="semantic-kernel", run_id="", checkpoint_id=""),
    ...
    def rollback(self, project, applied):
        ...
        pristine = Path(__file__).parent.parent.parent / "_fixtures" / "semantic_kernel" / "payment_agent.py"
